...
def generate_pdf():
    ...
    elements.append(Paragraph(f"<b>Transcript:</b> {text}", style=getSampleStyleSheet()['BodyText']))
    # Add EEG Analysis Result
    elements.append(Paragraph(" Suggestion"))
    elements.append(Paragraph(f"<b>Suggestion:</b> {suggestion}", style=getSampleStyleSheet()['BodyText']))
...
def main():
    global result, transcript, video_analysis_result, audio_analysis_result, eeg_analysis_result,analysis, suggestion, text

    st.set_page_config(layout="wide")
    st.title("Mind AI 🧠")
    st.subheader("Nurturing Minds!!")
    st.markdown("""
                        -Put on your EEG \n
    ...
    if st.button("Begin"):
        ...
        analysis = assist()
        st.write("Intant suggestion : \n")
        st.write(instant(analysis))
        suggestion=suggest(analysis)
        generate_pdf()
        st.success("The analysis is sent to the expert team and they will reach you back!")
